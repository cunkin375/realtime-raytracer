...
dRay Camera::GetRay(std::size_t i, std::size_t j) const
{
    ...
    auto ray_time = Math::Rand::GenerateNumberInUnitInterval<f64>();
...
dVector3 Camera::SampleSquare() const
{
    ...
    return dVector3{ Rand::GenerateNumberInUnitInterval<f64>() - 0.5,
                     Rand::GenerateNumberInUnitInterval<f64>() - 0.5, 0 };
}
