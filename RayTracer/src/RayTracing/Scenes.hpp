...
inline RenderContext<BVH<Sphere>> InitiateThreeSphereScene(dVector2 viewport = {})
{
    ...
    auto left_sphere_bubble =
        Sphere{ dPoint3{ -1, 0, -1 }, Radius{ 0.4 },
                Material{ std::in_place_type<Dielectric>, RefractionIndex{ 1.00 / 1.50 } } };

    auto right_sphere =
        Sphere{ dPoint3{ 1, 0, -1 }, Radius{ 0.5 },
...
inline World InitiateTwoSphereScene()
{
    ...
    auto sphere_left = Sphere{ dPoint3{ -R, 0, -1 }, Radius{ R },
                               Material{ std::in_place_type<Lambertian>, dColor{ 0, 0, 1 } } };

    auto sphere_right = Sphere{ dPoint3{ R, 0, -1 }, Radius{ R },
                                Material{ std::in_place_type<Lambertian>, dColor{ 1, 0, 0 } } };
...
inline RenderContext<BVH<Sphere>> InitiateManySphereScene()
{
    ...
    auto ground_object =
        Sphere{ dPoint3{ 0, -1000, 0 }, Radius{ 1000 },
                Material{ std::in_place_type<Lambertian>,
                          Textures::Checker<SolidColor>{ Scale{ 1.0 }, dColor{ 0.2, 0.3, 0.1 },
                                                         dColor{ 0.9, 0.9, 0.9 } } } };
    ...
    // Init Camera
    auto camera =
        Camera{ AspectRatio{ 16.0 / 9.0 }, ImageWidth{ 1080 }, SamplesPerPixel{ 25 }, VerticalFOV{ 20 } };
...
inline RenderContext<BVH<Sphere>> InitiateTwoCheckeredSpheres()
{
    ...
    world.Add(
        Sphere{ dPoint3{ 0, -10, 0 }, 10, Material{ std::in_place_type<Lambertian>, checker_pattern } });
