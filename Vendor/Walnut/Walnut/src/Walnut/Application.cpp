...
static void SetupVulkan(const char **extensions, uint32_t extensions_count, bool enable_validation_layers)
{
    ...
    // Create Vulkan Instance
    {
        ...
        if (vkCreateDebugUtilsMessengerEXT != NULL)
        {
            ...
            debug_messenger_ci.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT
            ...
            err = vkCreateDebugUtilsMessengerEXT(g_Instance, &debug_messenger_ci, g_Allocator,
                                                 &g_DebugMessenger);
            check_vk_result(err);
        }
#else
...
static void FrameRender(ImGui_ImplVulkanH_Window *wd, ImDrawData *draw_data)
{
    ...
    }
    {
        ...
        // These use g_MainWindowData.FrameIndex and not s_CurrentFrameIndex because they're tied to the
    ...
    while (!glfwWindowShouldClose(m_WindowHandle) && m_Running)
    {
...

        {
            ...
            if (m_Specification.CustomTitleBar)
            {
                ...
                {
                    const float button_width        = 30.f;
                    const float button_height       = ImGui::GetFrameHeight();
                    const float button_spacing      = ImGui::GetStyle().ItemSpacing.x;
                    const float total_buttons_width = (button_width * 3.f) + (button_spacing * 2.f);
                    const float right_margin        = 8.f;

                    if (m_MenubarCallback)
                        m_MenubarCallback();

                    // drag region in title bar
                    ImVec2 titlebar_min = ImGui::GetCursorScreenPos();
                    float  titlebar_drag_width =
                        (window_position.x + window_size.x - total_buttons_width - right_margin)
                        - titlebar_min.x;

                    if (titlebar_drag_width > 0.f)
                    {
                        ImGui::InvisibleButton("##titlebar_drag",
                                               ImVec2{ titlebar_drag_width, button_height });
                        if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left))
                        {
                            ImVec2 delta = ImGui::GetMouseDragDelta(ImGuiMouseButton_Left);
                            int    window_x, window_y;
                            glfwGetWindowPos(m_WindowHandle, &window_x, &window_y);
                            glfwSetWindowPos(m_WindowHandle, window_x + static_cast<int>(delta.x),
                                             window_y + static_cast<int>(delta.y));
                            ImGui::ResetMouseDragDelta(ImGuiMouseButton_Left);
                        }
                    }

                    // move the window if it is being dragged

                    // window control buttons (right aligned)
                    ImGui::SetCursorPosX(window_size.x - total_buttons_width - right_margin);
